...
class CostumerController extends BaseController {
	...
	public function getIndex()
	{
		...
		$this->layout->nest('content','costumer.index',$this->data)
	...
	function getAdd( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->layout->nest('content','costumer.form',$this->data)->with('menus', $this->menus );	
	}
	...
	function getShow( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['access']		= $this->access;
		$this->layout->nest('content','costumer.view',$this->data)->with('menus', $this->menus );	
	}	
	...
	function postSave( $id =0)
	{
		...
		if ($validator->passes()) {
			...
			$redirect = (!is_null(Input::get('apply')) ? 'costumer/add/'.$id.'?md='.$md.$trackUri :  'costumer?md='.$md.$trackUri );
			return Redirect::to($redirect)->with('message', SiteHelpers::alert('success',Lang::get('core.note_success')));
		} else {
			$md = str_replace(" ","+",Input::get('md'));
			return Redirect::to('costumer/add/'.$id.'?md='.$md)->with('message', SiteHelpers::alert('error',Lang::get('core.note_error')))
	...
	public function postDestroy()
	{
		...
		return Redirect::to('costumer?md='.Input::get('md'));
	}			
